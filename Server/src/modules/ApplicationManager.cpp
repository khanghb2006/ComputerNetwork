...
// split command line to extract exe path
...
// scan folder to find the first .exe file
...
std::vector<ApplicationInfo> ApplicationManager::listApplicationWindows() {
	...
	// get PID list of applications with visible windows
...
void EnumerateRegistryKey(HKEY hBaseKey, const std::wstring& subPath, std::vector<ApplicationInfo>& out) {
    ...
    while (true) {
		...
		// if exePath is not found, continue
        if (!exePath.empty()) {
            out.emplace_back(
                0,
...
std::optional<int> ApplicationManager::startApplication(
	...
	// approve only .exe files that really exist
    if (appW.length() < 4 ||
...
bool ApplicationManager::stopApplication(const std::string& appNameOrPath) {
	...
	bool processTerminated = false; // flag to track if any process was terminated
