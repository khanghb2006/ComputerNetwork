...
function handleServerMessage(msg) {
    ...
    if (msg.startsWith("IMAGE_FRAME:") || imgBuffer.startsWith("IMAGE_FRAME:")) {
        ...
        if (imgBuffer.includes("__EOF__")) {
            let base64 = imgBuffer.replace("IMAGE_FRAME:", "").replace("__EOF__", "").replace(/\s+/g, "");
            imgBuffer = "";
            renderImage(base64);
        }
        return;
    ...
    if (typeof ws !== "undefined" && ws.url) {
        serverIP = ws.url.split("/")[2].split(":")[0];
    }

    // URL cố định đúng format bạn muốn
    lastVideoUrl = `http://${serverIP}:8080/Video/record.mp4`;

    console.log("Video URL:", lastVideoUrl);
...
function connectToAgent() {
    ...
    // Lấy đúng Port từ giao diện (mặc định là 8081 cho Server của bạn)
    const port = document.getElementById("portInput").value; 
    
    if(!selectedIP) {
        alert("Please select a target agent first.");
        return;
    }

    // Gọi hàm kết nối thật trong websocket.js
    connectWS(selectedIP); 
    
    // Ẩn bảng chọn sau khi kết nối
